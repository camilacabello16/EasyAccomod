...
<c:url var="companyChatApi" value="/api-chat"/>
<c:url var="redirectUrl" value="/company-chat?id=${adminid}"></c:url>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Trò chuyện cùng chủ trọ</title>
</head>
<body>
	<main>
	                                    ...
	                                    <div class="content-chat-user" id="chatFrame">
	                                        <c:forEach var="chat" items="${chats.listResult}">
	                                    		<c:if test="${chat.role == 1}">
	                                    			<div class="msg-sent-company">
			                                            <p>${chat.content}</p>
			                                        </div>
	                                    		</c:if>
	                                    		<c:if test="${chat.role == 0}">
	                                    			<div class="msg-received-company">
			                                            <p>${chat.content}</p>
			                                        </div>
	                                    		</c:if>
	                                    	</c:forEach>
	...
	<script type="text/javascript">
		...
		$('#btnSendMsg').click(function(e){
			...
			formSendMsg.push({name: "adminId", value: adminId});
